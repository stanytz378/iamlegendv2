...
// Use environment variables for links, fallback to your IAMLEGEND links
const CHANNEL_LINK = process.env.CHANNEL_LINK || 'https://whatsapp.com/channel/0029Vb7fzu4EwEjmsD4Tzs1p';
const GROUP_LINK = process.env.GROUP_LINK || 'https://chat.whatsapp.com/J19JASXoaK0GVSoRvShr4Y';
...
async function handleMessages(sock, messageUpdate) {
    try {
        ...
        if (/^[1-9]$/.test(userMessage) || userMessage === 'surrender') {
            const tttContext = { channelInfo };
            await handleTicTacToeMove(sock, chatId, senderId, userMessage, tttContext);
            return;
        }
